...
<script>
	...
	async created() {
		...
		const actions = await this.$axios.get('https://fairplay-rp.ro/api/recentactions')
		this.RecentActions = actions.data.sort((x, y) => +new Date(y.time) - +new Date(x.time)).slice(0, 5);
		let users = await this.$axios.get('https://fairplay-rp.ro/api/users')

		this.UsersData = users.data;
		this.TopPlayers = this.UsersData.sort((x, y) => y.hoursPlayed - x.hoursPlayed).slice(0, 5);
		...
		$.getJSON("https://servers-frontend.fivem.net/api/servers/single/2e3lgd", function (data) {
			$("#dashboard-players").text(data['Data']['clients'] + '/' + data['Data']['sv_maxclients']);
		});
	},
	computed: {
		IntregistratedUsers() {
		...
		GetOnlinePlayers() {
			$.getJSON("https://servers-frontend.fivem.net/api/servers/single/2e3lgd", async function (data) {
				return await data['Data']['clients'] + '/' + data['Data']['sv_maxclients'];
			});
		},
	}
};
